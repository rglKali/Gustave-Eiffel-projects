...
class CustomCanvas:
    ...
    def __init__(self, width, height, fullscreen = False, refresh_rate=100, events=None):
        # interval of the canvas
        ...
        self.width = self.root.winfo_screenwidth() if fullscreen else width
        self.height = self.root.winfo_screenheight() if fullscreen else height
    ...
    def resize(self, width, height, fullscreen):
        self.root.attributes('-fullscreen', fullscreen)
        self.width = self.root.winfo_screenwidth() if fullscreen else self.width
        self.height = self.root.winfo_screenheight() if fullscreen else self.height
...
def cree_fenetre(largeur, hauteur, fullscreen=False, frequence=100):
    """
...
def redimensionner_fenetre(largeur=None, hauteur=None, fullscreen=None):
    if __canevas is not None:
        raise FenetreDejaCree(
            'La fenêtre a déjà été crée avec la fonction "cree_fenetre".')
    else:
        __canevas.resize(largeur, hauteur, fullscreen)
